<template>
  <v-app-bar elevation="2" class="app-navbar position-relative" density="comfortable">
    ...
    <v-list nav class="d-flex">
      <v-list-item>
        <router-link to="/" class="text-decoration-none text-black font-weight-bold d-flex">
          <v-img src="../../assets/logo.svg" width="20"></v-img>
          <span class="ms-1">My Blogs</span>
        </router-link>
      </v-list-item>

      <v-list-item class="hidden-sm-and-down ms-7" v-for="item in menuItems2" :key="item.title" :title="item.title"
